...
Group all test cases into these 6 categories:
...
**Bare Minimum** — the absolute most critical tests that MUST pass before shipping; if any of these fail the feature is broken. Keep this list short (2–5 tests max) — only the highest-stakes, can't-miss scenarios.
**Affected Legacy** — tests for existing legacy features, older flows, or unrelated areas that could silently break due to shared code, database schema changes, config changes, or side effects from this diff. Focus on areas not obviously connected to the change.

For each test case, produce:
- `id`: sequential string like `TC-001`, `TC-002`, etc. (global across all categories)
- `category`: one of `Happy Path`, `Edge Cases`, `Regression`, `Negative`, `Bare Minimum`, `Affected Legacy`
...
- Replace `{{HAPPY_COUNT}}`, `{{EDGE_COUNT}}`, `{{REGRESSION_COUNT}}`, `{{NEGATIVE_COUNT}}`, `{{BAREMINIMUM_COUNT}}`, `{{LEGACY_COUNT}}` with counts by category
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .summary-card.bare-minimum { border: 2px solid #e53e3e; }
  .summary-card.bare-minimum .value { color: #e53e3e; }
  .summary-card.legacy .value { color: #6b46c1; }
...
<div class="summary-bar">
  ...
  <div class="summary-card bare-minimum"><div class="label">Bare Minimum</div><div class="value">{{BAREMINIMUM_COUNT}}</div></div>
  <div class="summary-card legacy"><div class="label">Affected Legacy</div><div class="value">{{LEGACY_COUNT}}</div></div>
</div>
{{CATEGORY_SECTIONS}}
<script>
...
✓ Test cases saved to: ./test-cases/{name}.html
  ...
  Categories: {HAPPY_COUNT} Happy Path · {EDGE_COUNT} Edge Cases · {REGRESSION_COUNT} Regression · {NEGATIVE_COUNT} Negative · {BAREMINIMUM_COUNT} Bare Minimum · {LEGACY_COUNT} Affected Legacy
```
